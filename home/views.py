...
import os
import time
from .models import Product, Cart, MyOrder, Merchant, Review, Chat, Message
...
def inbox(request):
    ...
    chats = Chat.objects.filter(Q(user1=profile) | Q(user2=profile)).annotate(last_message_time=Max('messages__timestamp')).order_by('-last_message_time')
    return render(request, 'inbox.html', {'chats': chats, 'profile': profile})
...
def chat_view(request, chat_id):
    ...
    messages = Message.objects.filter(
        Q(sender=chat.user1, receiver=chat.user2) | Q(sender=chat.user2, receiver=chat.user1)
    ).order_by('timestamp')

    for message in messages.filter(receiver=profile):
        message.seen = True
        message.save()

    return render(request, 'chat.html', {'chat': chat, 'messages': messages, 'profile': profile})
...
def start_chat(request, profile_id):
    user1 = request.user
    user2 = get_object_or_404(User, id=profile_id)
    
    chat, created = Chat.objects.get_or_create(
        user1=user1 if user1.id < user2.id else user2,
        user2=user2 if user2.id > user1.id else user1
    )
...
def superuser_list(request):
    ...
    return render(request, 'superuser_list.html', {'superusers': superusers})
